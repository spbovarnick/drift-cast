...
export default function Card({ riverData }) {
   const [conditionCheck, setConditionCheck] = useState()
    ...
    if (riverData) {
        renderCard = <div className={`border-2 border-${conditions.color}`}>
            <p className={`text-${conditions.color}`}>{riverData.name}</p>
            <div>
                <iframe
                    className=""
                    loading="lazy"
                ...
                />
                <div className={`h-52 w-52 border-2 border-${conditions.color} rounded-full bg-${conditions.color}`}>
                </div>
            </div>
